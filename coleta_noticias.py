...
def buscar_dados():
    ...
    # 2. Fonte: Agência Brasil via RSS
    try:
    ...
    try:
        logging.info("[4/6] Acessando FAPEAM - Editais (Web Scraping)...")
        ...
        for item in destaques:
            ...
            if titulo and link_tag and 'href' in link_tag.attrs:
                ...
                noticias.append({"fonte": "FAPEAM", "titulo": titulo, "resumo": "Processado via FAPEAM.", "link": link_final})
                count += 1
    except Exception as e:
    ...
    try:
        logging.info("[5/6] Acessando Rede GIFE (Web Scraping)...")
        ...
        for item in destaques:
            ...
            if titulo and link_tag and 'href' in link_tag.attrs:
                ...
                noticias.append({"fonte": "GIFE", "titulo": titulo, "resumo": "Processado via GIFE.", "link": link_final})
                count += 1
    except Exception as e:
    ...
    try:
        ...
        for entry in mds_feed.entries[:4]:
             noticias.append({"fonte": "MDS - Gov.br", "titulo": entry.title, "resumo": entry.summary, "link": entry.link})
    except Exception as e:
...
def processar_com_gemini(dados_brutos):
    logging.info("Iniciando IA (Gemini 2.5 Flash)...")
    if not GEMINI_API_KEY: raise ValueError("GEMINI_API_KEY não localizada.")
    client = genai.Client(api_key=GEMINI_API_KEY)
    
    prompt = f"""Você é o Cérebro de IA da AME-AMAZÔNIA. Analise os dados e extraia oportunidades focadas em: Multiempreendedorismo, Inclusão Produtiva, Meio Ambiente, Empreendedorismo Verde, Educação (Jovem Aprendiz), Assistência Social e Tecnologia. Gere JSON estrito conforme formato definido anteriormente.
    Dados coletados: {json.dumps(dados_brutos, ensure_ascii=False)}"""
...
def main():
    try:
        ...
        # Correção para horário de Manaus (UTC-4)
        ...
        logging.info("Processo concluído com êxito!")
    except Exception as e:
